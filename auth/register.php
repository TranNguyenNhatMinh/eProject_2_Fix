...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['register'])) {
    ...
    if (empty($username) || empty($email) || empty($password)) {
        ...
        if ($result['success']) {
            $redirect = $_GET['redirect'] ?? '';
            if (!empty($redirect) && strpos($redirect, '://') === false && strpos($redirect, '//') !== 0 && preg_match('#^[a-zA-Z0-9_./\-?=&]+$#', $redirect)) {
                header('Location: ../' . ltrim($redirect, '/'));
            } else {
                header('Location: ../index.php');
            }
                        ...
                        <div class="auth-bar-right">
                            ...
                            <?php if ($error): ?>
                                <div class="alert alert-danger">
                                    ...
                                    <div><strong>Error:</strong> <?php echo htmlspecialchars($error, ENT_QUOTES, 'UTF-8'); ?></div>
                                </div>
                            <?php endif; ?>
                            <?php if ($success): ?>
                            ...
                            <div class="auth-footer">
                                <?php $loginUrl = 'login.php' . (isset($_GET['redirect']) ? '?redirect=' . urlencode($_GET['redirect']) : ''); ?>
                                <p class="mb-0">Already have an account? <a href="<?php echo htmlspecialchars($loginUrl); ?>" class="auth-link">Sign In</a></p>
                            </div>
                        </div>
                    </div>
                </div>
